...
import { posts } from '@/data'; // Adjust the import path as needed

const SingleDetailPost = () => {
  const router = useRouter();
  const searchParams = useSearchParams();
  const postId = searchParams.get('id'); // Get the post ID from the URL

  const [post, setPost] = useState<any>(null); // Replace 'any' with your Post type if you have one
  ...
  const [likes, setLikes] = useState<number>(post.likes || 0); // Adjust based on your data
  const [commentsCount, setCommentsCount] = useState<number>(post.commentsCount || 0); // Adjust based on your data
  const [liked, setLiked] = useState<boolean>(post.liked || false); // Adjust based on your data
  ...
  return (
    <section className="mb-26 bg-black-200 py-16" id="featured-posts">
      <div className="container mx-auto px-6 mt-[6rem]">
        ...
        <div className="flex flex-col md:flex-row items-center">
          <div className="relative w-full md:w-1/3 mb-4 md:mb-0">
            <Image src={post.imageUrl} alt={post.title} width={400} height={240} className="object-cover rounded-lg" />
            ...
            <div className="my-8 flex items-center">
              ...
              <div className="ml-4">
                ...
                <span className='flex text-center'>
                  <p className="text-xs text-gray-500 mr-2">{format(parseISO(post.author.date), 'PP')}</p>
                  <p className="text-xs text-gray-500">{format(parseISO(post.author.datetime), 'p')}</p>
                </span>
              </div>
            </div>
...
// // Import the posts data from Posts.tsx or wherever your data is defined
// import { posts } from '@/data'; // Adjust the import path as needed

// interface SingleDetailPostProps {
//   postId: string;
// }

// const SingleDetailPost: React.FC<SingleDetailPostProps> = ({ postId }) => {
//   const [post, setPost] = useState<any>(null); // Replace 'any' with your Post type if you have one

//   useEffect(() => {
//     if (postId) {
//       // Fetch or find the post by ID
//       const foundPost = posts.find((p) => p.id === parseInt(postId, 10));
//       setPost(foundPost || null);
//     }
//   }, [postId]);
...
//===================== Not fetching or displaying the fetched post
// 'use client';
...
// // Example posts data. Replace this with your actual data fetching logic.
// import { posts } from '@/data'; // Adjust the import path as needed
